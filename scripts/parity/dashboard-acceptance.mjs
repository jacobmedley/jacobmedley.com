...
try {
  ...
  assert.equal(results.motion.touch, 'focal-float')
  ...
  const initialTouch = await touchAnchor.evaluate(node => getComputedStyle(node).translate)
  await touch.waitForTimeout(700)
  assert.notEqual(await touchAnchor.evaluate(node => getComputedStyle(node).translate), initialTouch, 'touch animation advances')
